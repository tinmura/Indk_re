...
# 諸パラメータ
folder 	= "220526_ad50um_659_cv"						### フォルダ名
subst 	= "ad100um"				               		### 基板名
pH 	= [405,507,597,690]					                       	### pH
...
for i in range(len(pH)):
    for j in range(N):
        with open("{0:02}_pH{1}_m_{2}".format(i+1,pH[i],j+1)) as f_m:
            lst=list(csv.reader(f_m))
            #print(lst)
            for k in range(1,Vm+1,1):
                bias_V[k-1] = float(lst[k][0])
                for F in range(0,frq_N,1):
                    cs_raw[F][i][j][k-1] = float(lst[F*(abs(Vm)+1)+ k][2])
                    
        with open("{0:02}_pH{1}_p_{2}".format(i+1,pH[i],j+1)) as f_p:
            lst=list(csv.reader(f_p))
            for k in range(Vm+1,StepTime+1,1):
                bias_V[k-1] = float(lst[k-Vm][0])
                for F in range(0,frq_N,1):
                    #cs_p[F] = lst[(F-1)*(abs(Vm+1))+ (k-Vm)][2]
                    cs_raw[F][i][j][k-1] = float(lst[F*(Vp+1)+ (k-Vm)][2])
